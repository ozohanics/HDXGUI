...
public:
  SingleElementConfGenerator() : element_(nullptr), atom_count_(-1), log_threshold_(0.001) {}
...
class SumFormulaParser {
 ...
 private:
  ...
  void parseElement(ElementCounter& counter) {
    ...
    auto pos = n;
    if (!isupper(element.back())) throw ParseError("expected an element", n);
    while (!eof() && islower(peek()))
      element.push_back(nextChar());
  ...
  void parseFragment(ElementCounter& counter) {
    ...
    } else if (isupper(peek())) { parseSimpleFragment(counter); } else {
...
#pragma omp parallel for
  for (long i = 0; i < possible_adducts.size(); i++) {
...
#pragma warning( default : 26495 4267)