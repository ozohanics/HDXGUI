...
class MSReader {
 public:
  ...
  void writeFile(const char* c, MSFileFormat ff, MSObject& m, char* sha1Report=nullptr);

  bool readMGFFile(const char* c, Spectrum& s); //Note, no random-access of MGF files.
  bool readMGFFile(const char* c, Spectrum& s, bool readNext); //Note, no random-access of MGF files.
