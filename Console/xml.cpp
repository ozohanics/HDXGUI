...
XMLElement* XML::ImportRKey(IMPORTRKEYDATA* d) {
	...
	for (int i = 0; ; i++) {
		...
		ZXML<char>tmp2(si + 10);

		ts = 20000;
		if (RegEnumValue(pK, i, tmp1, &ts, 0, &ty,
			(unsigned char*)tmp2.operator char *() + 2, &si) != ERROR_SUCCESS)
			break; // end of values
...
XMLElement* XML::ImportDB(IMPORTDBPARAMS* db) {
	...
	// Open Database
	CoCreateInstance(CLSID_CADOConnection, 0, CLSCTX_ALL, IID_IADOConnection,
		(void**)&aC);
	if (!aC)
		goto E_0;

	if (db->provstr == 0)
		sprintf(str, "Provider=Microsoft.Jet.OLEDB.4.0;Data Source=%s",
		db->dbname);
	else
	...
	for (int iTable = 0; iTable < db->nTables; iTable++) {
		...
		for (unsigned int y = 0; y < (unsigned)TotalRecords; y++) {
			...
			for (int iVariables = 0; iVariables < RecordCount; iVariables++) {
				...
				if (Value.vt == VT_BSTR)
					WideCharToMultiByte(CP_UTF8, 0, Value.bstrVal, -1, str,
					1000, 0, 0);
				...
				int SL = wcslen(Name);
				ZXML<char>nam(SL*2 + 100);
				WideCharToMultiByte(CP_UTF8, 0, Name, -1, nam,
					SL*2 + 100, 0, 0);
				int IsToAdd = -1;
				for (int yAdds = 0; yAdds < db->Tables[iTable].nVariables;
				yAdds++) {
...
int XML::XMLGetValue(const char* section2, char* attr2, char* put2, int maxlen)
{
	...
	ZXML<char>section(y1 + 10);
	ZXML<char>attr(y2 + 10);
...
void XML::XMLSetValue(const char* section2, char* attr, char* put) {
	...
	XMLElement* r = root;
	XMLElement* rr = root;
	ZXML<char>section(strlen(section2) + 10);
...
char* XMLHelper::FindXMLClose(char* s) {
	...
	for (; ;) {
		...
		if ((*(a1 + 1) == '!' && strlen(a1) > 2) || *(a1 + 1) == '?')
			IsComment = true;

		if (*(a1 + 1) == '!' && strlen(a1) > 8 && strncmp(a1 + 1, "![CDATA[",
			8) == 0)
			IsCData = true;
...
XMLElement* XMLHelper::ParseElementTree(XMLHeader* hdr, XMLElement* parent,
	...
	for (; ;) {
		...
		*a3 = '<';
		...
		if ((*(a3 + 1) == '!' && strlen(a3 + 1) > 8 && strncmp(a3 + 1,
			"![CDATA[", 8) == 0)) // cdata
		{
			c2 = *a4;
			*a4 = 0;
		...
		if (*(a4 - 1) == '/') {
			...
			*a4 = 0;
			...
			if (parent)
				parent->AddElement(c);
			else ; // invalid. Can't have a < /> root
...
int XMLElement::InsertElement(unsigned int y, XMLElement* x) {
	...
	memmove((void*)(children + y + 1), (void*)(children + y),
		(childrennum - y)*sizeof(XMLElement*));
	children[y] = x;
	x->SetParent(this);
	childrennum++;
	return y;
...
XMLElement* XMLElement::GetElementInSection(const char* section2) {
	...
	ZXML<char>section(strlen(section2) + 1);
	strcpy(section, section2);
...
void XMLElement::printc(FILE* fp, XMLElement* root, int deep, int ShowAll,
	...
	if (TargetMode == 1)
		...
		root->GetElementName(b.operator char *() + strlen(b), SaveMode);
		DWORD dw = 0;
		RegCreateKeyExA(pKey, b, 0, 0, REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS,
			0, &pKey2, &dw);
#endif
#endif
	}
	...
	if (iY) {
		for (int i = 0; i < iY; i++) {
			...
			ZXML<char>Name(s1 + 10);
			ZXML<char>Value(s2 + 10);
...
#ifndef WINCE
					// create a value
					ZXML<char>VName(strlen(Name) + 10);
					sprintf(VName, "V%s", Name.operator char*());
					RegSetValueExA(pKey2, VName, 0, REG_SZ,
						(const BYTE*)Value.operator char *(),
						strlen(Value) + 1);
#endif
#endif
				}
				else if (TargetMode == 3) {
					ZXML<char>xy(strlen(Name)*2 + 100 + strlen(Value)*2);
					sprintf(xy, " %s=\"%s\"", Name.operator char*(),
						Value.operator char*());
					Write16String(fp, xy);
				}
				else // TM == 0
	...
	if ((!iC || ShowAll == 0) && /* SaveAstVariable == 0 && */ TotalContents ==
		0 && TotalComments == 0 && TotalCDatas == 0) {
		if (TargetMode == 1)
			sprintf(sp + strlen(sp), "/>\r\n");
		else if (TargetMode == 2); // Nothing :)
		else if (TargetMode == 3)
			fwrite(L"/>\r\n", 1, 8, fp);
		else // 0
				fprintf(fp, "/>\r\n");
	...
	if (ShowAll) {
		for (int i = 0; i < iC; i++) {
			if (TotalComments && (NextComment < TotalComments)) {
				while ((NextComment < TotalComments) && root->GetComments()
					[NextComment]->GetEP() <= i) {
					// print that comment now
					char* t =
						root->GetComments()[NextComment]->operator char *();
					ZXML<char>b(strlen(t) + deep + 200);
					...
					if (TargetMode == 1)
						...
						RegSetValueExA(pKey2, VName, 0, REG_SZ, (const BYTE*)t,
							strlen(t) + 1);
#endif
#endif
					}
			...
			if (TotalContents && (NextContent < TotalContents)) {
				while ((NextContent < TotalContents) && root->GetContents()
					[NextContent]->GetEP() <= i) {
					// print that content now
					// char* t = root->GetContents()[NextContent]->operator char *();
					int vx = root->GetContents()[NextContent]->GetValue(0);

					ZXML<char>b(vx + deep + 200);
					for (int i = 0; i < (deep + 1); i++)
						// strcat(b,"\t");
							strcat(b, DelimiterChar);
					// strcat(b,t);
					root->GetContents()[NextContent]->GetValue
						(b.operator char*() + strlen(b), SaveMode);
					strcat(b, "\r\n");
					...
					if (TargetMode == 1)
						...
						RegSetValueExA(pKey2, VName, 0, REG_SZ,
							(const BYTE*)b.operator char *(), strlen(b) + 1);
#endif
#endif
					}
					else if (TargetMode == 3)
				...
				if (TotalCDatas && (NextCData < TotalCDatas)) {
					while ((NextCData < TotalCDatas) && root->GetCDatas()
						[NextCData]->GetEP() <= i) {
						// print that CData now
						char* t =
							root->GetCDatas()[NextCData]->operator char *();
						ZXML<char>b(strlen(t) + deep + 200);
						for (int i = 0; i < (deep + 1); i++)
							// strcat(b,"\t");
								strcat(b, DelimiterChar);
...
#ifdef _WIN32
			if (TargetMode == 2)
				printc((FILE*)pKey2, root->GetChildren()[i], deep + 1, ShowAll,
				SaveMode, TargetMode);
			else
#endif
				printc(fp, root->GetChildren()[i], deep + 1, ShowAll, SaveMode,
				TargetMode);
		}
	}
...
void XMLElement::Export(FILE* fp, int ShowAll, int SaveMode, int TargetMode,
	XMLHeader* hdr, class XMLTransform* eclass, class XMLTransformData* edata) {
	// Export this element
	if (eclass == 0) {
		if (hdr)
		...
		int NewTargetMode = 1;
		if (hdr)
			hdr->Export((FILE*)ram.operator char *(), 0, NewTargetMode,
			eclass, edata);
		printc((FILE*)ram.operator char *(), this, 0, ShowAll, SaveMode,
			NewTargetMode);
		if (hdr)
			hdr->Export((FILE*)ram.operator char *(), 1, NewTargetMode,
			eclass, edata);
		...
		S = strlen(ram);
		int nS = eclass->Encrypt(ram.operator char *(), S, 0,
			yy.operator char *(), S + 100, 0);
...
void XMLElement::SortVariables(int(*fcmp)(const void *,
	const void *)) {
	// to all Variables
...
XMLElement* XMLElement::Duplicate(XMLElement* par) {
	...
	y = GetContentsNum();
	for (int i = 0; i < y; i++) {
		nX->AddContent(GetContents()[i]->Duplicate(),
			GetContents()[i]->GetEP());
	}
	...
	y = GetCommentsNum();
	for (int i = 0; i < y; i++) {
		nX->AddComment(GetComments()[i]->Duplicate(),
			GetComments()[i]->GetEP());
	}
...
int XMLElement::GetElementFullName(char* x, bool NoDecode) {
	ZXML<char>fel(5000); // full element name store here
...
int XML::PartialLoad(const char* file, XMLPARTIALLOADOPTIONS* d) {
		...
		for (int i = 0; i < Y; i++) {
			if (b[i] == '<' && IsQuote == false) {
				if (b[i + 1] == '?')
					...
					CurrentDeep++;
					if (CurrentDeep == d->deep && PositionStarting < 0)
						PositionStarting = i;
					IsHdr = false;
				}
			}
			...
			if (b[i] == '>' && b[i - 1] == '/') {
				CurrentDeep--;
				if (CurrentDeep < d->deep && PositionEnding < 0) {
					PositionEnding = i;
	...
	if (PositionStarting < 0 || PositionEnding < 0)
...
int XML::Load(const char* file, int LoadMode, XMLTransform* eclass,
	...
	else if (LoadMode == 1) // memory buffer
	{
		f = 0;
	...
	else if (LoadMode == 2) // url
	{
		...
		hI = InternetOpen(_T("XML Library"), INTERNET_OPEN_TYPE_PRECONFIG,
			0, 0, 0);
		ZXML<char>fx(1000);
		...
		if (hI) {
			...
			if (hRead) {
				...
				for (; ;) {
					...
					HANDLE hF = CreateFileA(fx, GENERIC_WRITE, 0, 0,
						OPEN_ALWAYS, 0, 0);
...
int XMLElement::SpaceForElement(unsigned int i) {
	...
	// make more space
	ZXML<XMLElement*>oldp(childrennum);
...
int XMLElement::SpaceForVariable(unsigned int i) {
	...
	ZXML<XMLVariable*>oldp(variablesnum);
...
int XMLElement::SpaceForComment(unsigned int i) {
	...
	ZXML<XMLComment*>oldp(commentsnum);
	memcpy(oldp, comments, commentsnum*sizeof(XMLComment*));
...
unsigned int XMLElement::GetAllChildren(XMLElement** x, unsigned int deep) {
	...
	for (unsigned int i = 0; i < childrennum && deep != 0; i++) {
		C += children[i]->GetAllChildren(x + C, deep == 0xFFFFFFFF ? deep :
			(deep - 1));
		x[C++] = children[i];
	}
...
unsigned int XMLElement::GetAllChildrenNum(unsigned int deep) {
	int C = 0;
	for (unsigned int i = 0; i < childrennum && deep != 0; i++) {
		C += children[i]->GetAllChildrenNum(deep == 0xFFFFFFFF ? deep :
			(deep - 1));
	}
	C += childrennum;
	return C;
...
// XMLContent class
XMLContent::XMLContent(XMLElement* p, int ElementPosition, const char* ht,
	bool NoDecode) {
	parent = p;
	ep = ElementPosition;
	c = 0;
...
XMLHeader& XMLHeader:: operator = (XMLHeader & h) {
	...
	for (int i = 0; i < yC; i++) {
		AddComment(h.GetComments()[i]->Duplicate(),
			h.GetComments()[i]->GetEP());
	}
...
void XMLHeader::SetEncoding(char* e) {
	...
	ZXML<char>nt(1000);
	sprintf(nt,
		"<?xml version=\"1.0\" encoding=\"%s\" standalone=\"yes\" ?>", e);
	int sl = strlen(nt) + 1;
	delete[]hdr;
	hdr = new char[sl];
	memset(hdr, 0, sl);
...
XMLHeader* XMLHeader::Duplicate() {
	...
	for (int i = 0; i < y; i++) {
		hz->AddComment(GetComments()[i]->Duplicate(),
			GetComments()[i]->GetEP());
	}
...
void XMLHeader::Export(FILE* fp, int SaveMode, int TargetMode,
	class XMLTransform* eclass, class XMLTransformData* edata) {
	if (TargetMode == 1) {
		if (SaveMode == 0)
			sprintf((char*)fp, "%s\r\n", hdr);
	...
	{
		if (SaveMode == 0) {
			ZXML<char>txt(strlen(hdr)*2 + 100);
			sprintf(txt, "%s\r\n", hdr);
...
int XMLVariable::GetBinaryValue(char* data) {
	...
	ZXML<char>bdata(aL + 100);
	GetValue(bdata);

	ZXML<char>oout(aL + 1000);
...
int XML::XMLQuery(const char* rootsection, const char* expression,
	XMLElement** rv, unsigned int deep) {
...
int XMLElement::XMLQuery(const char* expression2, XMLElement** rv,
	unsigned int deep) {
	ZXML<char>expression(strlen(expression2) + 1);
	...
	ZXML<XMLElement*>allelements(C + 1);
	r->GetAllChildren(allelements, deep);
	ZXML<int>positives(C);
	for (int i = 0; i < C; i++)
		positives[i] = 1;
	...
	for (; ;) {
		...
		*a1 = ' ';
		...
		for (int y = 0; y < C; y++) {
			...
			else if (strcmp(item1, "!") == 0) {
				allelements[y]->GetElementFullName(ItemToMatch, 0);
				if (XML::TestMatch(ItemToMatch, comp, item2) == 0)
					positives[y] = 0;
			}
			else if (strncmp(item1, "~", 1) == 0) {
...
// Global functions
ZXML<char> * XML::ReadToZ(const char* file, XMLTransform* eclass,
	class XMLTransformData* edata) {
	FILE* fp = fopen(file, "rb");
	if (!fp)
		return 0;
	...
	if (eclass == 0) {
		...
		// eclass->Prepare(edata);
		eclass->Decrypt(yy.operator char *(), S, 0,
			(*y).operator char *(), S, 0);
	}
	...
	if (yy[0] == 0xFF && yy[1] == 0xFE) {
		...
		ZXML<char> *nyy = new ZXML<char>(S * 4 + 32);
...
int XMLGetString(const char* section, const char* Tattr, const char* defv,
	char*out, const int maxlen, const char* xml, XML* af) {
	int Z = 0;
	if (!af) {
		XML f(xml);
...
int XMLSetString(const char* section, const char* Tattr, char*out,
	const char* xml, XML* af) {
	if (!af) {
		XML f(xml);
...
int XMLSetString(const char* section, const char* Tattr, wchar_t*out,
	const char* xml, XML* af) {
	// Convert to UTF-8
	int S = wcslen(out);
	ZXML<char>ut(S*2 + 1000);
...
int XMLRenameElement(const char* section, const char* newname, const char* xml,
	XML* af) {
	bool C = false;
	if (!af) {
		C = true;
...
int XMLGetInt(const char* item, const char* attr, const int defv,
	const char* xml, XML* af) {
	ZXML<char>i(100);
	ZXML<char>id(100);
	sprintf(id, "%u", defv);
...
__int64 XMLGetInt64(const char* item, const char* attr, const __int64 defv,
	const char* xml, XML* af) {
	ZXML<char>i(100);
	ZXML<char>id(100);
	sprintf(id, "%Lu", defv);
...
double XMLGetFloat(const char* item, const char* attr, const double defv,
	const char* xml, XML* af) {
	ZXML<char>a1(30);
	sprintf(a1, "%f", defv);
...
int XMLGetBinaryData(const char* item, const char* attr, const char* defv,
	char*out, const int maxlen, const char* xml, XML* af) {
	ZXML<char>bdata(maxlen*5 + 1000);
...
int XMLSetFloat(char* section, char* attr, double v, char* xml, XML* af) {
	char a[20] = {0};
	sprintf(a, "%f", v);
	return XMLSetString(section, attr, a, xml, af);
}
...
int XMLSetBinaryData(char* section, char* attr, char* data, int len, char* xml,
	XML* af) {
	char* a = new char[len * 5 + 1000];
	memset(a, 0, len*5 + 1000);
...
int XMLGetAllVariables(char* section, vector<char*> * vnames,
	vector<char*> * vvalues, char*xml) {
	XML f(xml);
...
int XMLSetStringA(const char* item, const char* attr, const char*v,
	const char* xml) {
...
int XMLSetStringW(const char* item, const char* attr, const wchar_t*v,
	const char* xml) {
...
int XMLGetStringA(const char* section, const char* Tattr, const char* defv,
	char*out, const int maxlen, const char* xml, XML* aF, int CP) {
	int S = maxlen * 2 + 1000;
	ZXML<char>a1(S);
...
int XMLGetStringW(const char* section, const char* Tattr, const wchar_t* defv,
	wchar_t*out, const int maxlen, const char* xml, XML* aF, int CP) {
	//
	return 0;
}
